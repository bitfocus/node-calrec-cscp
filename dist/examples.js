...
function setupEventListeners(client) {
	...
	client.on("auxRoutingChange", (auxId, routes) => {
		...
		logWithTimestamp(
			`Aux ${auxId} routing changed: ${activeRoutes} active routes`,
		);
	});
	client.on("mainRoutingChange", (mainId, routes) => {
		const activeRoutes = routes.filter((r) => r).length;
		logWithTimestamp(
			`Main ${mainId} routing changed: ${activeRoutes} active routes`,
		);
	});
	...
	client.on("faderAssignmentChange", (assignment) => {
		logWithTimestamp(
			`Fader ${assignment.faderId} assignment changed:`,
			assignment,
		);
	});
	client.on("mainLevelChange", (mainId, level) => {
		...
		logWithTimestamp(
			`Main ${mainId} level changed: ${level} (${levelDb.toFixed(1)}dB)`,
		);
	});
	...
	client.on("unsolicitedMessage", (message) => {
		const commandName = Object.entries(protocol_1.COMMANDS).find(
			([_k, v]) => v === message.command,
		)?.[0];
		if (!commandName) {
			...
			if (unknownCommandCount <= 3) {
				logWithTimestamp(
					`Unknown unsolicited message: Command 0x${message.command.toString(16)}, Data: ${message.data.toString("hex")}`,
				);
...
async function runLevel1Examples(client) {
	...
	try {
		...
		try {
			...
			logWithTimestamp(
				`Fader 1: Level ${faderLevel}, ${faderLevelDb.toFixed(1)}dB`,
			);
		} catch (error) {
...
async function main() {
	...
	logWithTimestamp(
		`Starting Calrec CSCP examples (Level: ${level}, Host: ${host}:${port})`,
	);
